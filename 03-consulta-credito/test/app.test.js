...
describe('Testes de Integração', () => {
  ...
  test('Trabalho prático - Atividade 03: GET /cliente with at least 1 client', async () => {
    ...
    expect(res.body).toHaveLength(clientes.length);

    //TearDown
    if (setUp) {
      await db.cliente.destroy({where: {CPF: clienteJoao.CPF}})
    }
  })

  test('Trabalho prático - Atividade 03: GET /cliente with no client', async () => {
    
    //SetUp
    let setUp = false

    //Given
    let clientes = await db.cliente.findAll({ raw: true })
    if (clientes && clientes.length) {
      await db.cliente.destroy({ where: {} })
      setUp = true
    }
